...
<template>
    <div
        class="relative w-full flex flex-row items-center justify-between gap-4 bg-neutral-50 p-5 rounded-lg border-[3px] border-black">
        <transition name="edit-fade" mode="out-in" @after-enter="handleAfterEnter">
            <div v-if="!isEditing" class="flex-1 min-w-0 max-h-28 overflow-y-auto pr-2">
                <p class="text-black text-xl font-semibold break-words whitespace-pre-wrap">{{ card.text }}</p>
            </div>
            <textarea v-else ref="editInput" v-model="editingText" rows="2" @input="handleTextareaInput"
                @keydown.enter.prevent="save()" @blur="save()"
                class="flex-1 min-w-0 text-black px-4 py-2 text-xl font-semibold bg-transparent border-none focus:ring-0 resize-none break-words whitespace-pre-wrap overflow-hidden"></textarea>
        </transition>
