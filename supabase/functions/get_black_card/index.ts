...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body || !body.room_id || body.round === undefined) {
      return new Response(JSON.stringify({ error: 'Missing room_id or round' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!blackCards || blackCards.length === 0) {
      return new Response(JSON.stringify({ error: 'No black cards remaining' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    return new Response(JSON.stringify("success"), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
